...
const BlogPost = () => {
    ...
    if (!currentPost) return <div className="blog-not-found">Статья не найдена</div>;

    return (
        <div className="blog-post-detail">
            <Link to="/blog" className="back-link">← Назад к списку</Link>
            
            <h1>{currentPost.title}</h1>
            <p className="meta">
                Автор: {currentPost.author} | {new Date(currentPost.created_at).toLocaleDateString('ru-RU')}
            </p>
            ...
            <div className="blog-content">
                {currentPost.blocks.map((block: BlogBlock) => {
                    ...
                    if (block.block_type === 'text') {
                        return <p key={block.id} className="blog-text">{block.text_content}</p>;
                    }
                    
                    // Цитата
                    if (block.block_type === 'quote') {
                        return <blockquote key={block.id} className="blog-quote">{block.text_content}</blockquote>;
                    }
                    
                    // Изображение — с плавающим выравниванием
                    if (block.block_type === 'image') {
                        return (
                            <figure key={block.id} className="blog-image-figure">
                                ...
                                />
                                {block.image_alt && (
                                    <figcaption className="blog-image-caption">{block.image_alt}</figcaption>
                                )}
                            </figure>
                        );
                    }
                    
                    // Видео — с вертикальным форматом
