...
const Checkout = () => {
    ...
    const handlePayment = async () => {
        ...
        if (!address.full_name || !address.phone || !address.city || !address.street || !address.house) {
            alert('Пожалуйста, заполните все обязательные поля адреса');
            return;
        }

        // Проверка данных карты (упрощённая)
...
/>
                            <div>
                                ...
                                <p>{item.quantity} шт. × {item.price === 0 ? 'бесплатно' : `${item.price} ₽`}</p>
                            </div>
                        </div>
                    ))}
