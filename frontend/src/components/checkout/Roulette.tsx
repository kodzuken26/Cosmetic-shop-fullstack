...
import { fetchRouletteProducts, startSpin, stopSpin, closeRoulette } from '../../store/slices/rouletteSlice';
    ...
    const spin = () => {
        ...
        const interval = setInterval(() => {
            ...
            if (counter > 20) {
                ...
                dispatch(addToCart({ product_id: prizeProduct.id, quantity: 1, price: 0 }));
                
                // Сохраняем в localStorage, что пользователь уже крутил
                localStorage.setItem('roulette_spun', 'true');
                setHasSpun(true);
                
                setTimeout(() => {
                    alert(`🎉 Вы выиграли: ${prizeProduct.name}! Он уже добавлен в корзину бесплатно. 🎉`);
                }, 200);
            }
        }, 100);
    };
        ...
        <div className="roulette-overlay">
            <div className="roulette-modal" ref={modalRef}>
                ...
                <div className="roulette-grid">
                    {products.map((p, idx) => (
                        <div key={p.id} className={`roulette-item ${highlightIndex === idx ? 'highlight' : ''}`}>
                ...
                {prize && !hasSpun && (
                    <div className="prize">
                        🎉 Ваш приз: <strong>{prize.name}</strong> (бесплатно!) 🎉
                    </div>
                )}
            </div>
        </div>
