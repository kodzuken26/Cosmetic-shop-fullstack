...
const Cart = () => {
  ...
  const { items, total_price, total_items, loading, error } = useTypedSelector((state) => state.cart);
...
/>
            ...
            <div className="cart-item-quantity">
              ...
              <span>{item.quantity} шт. × {item.price === 0 ? 'бесплатно' : `${item.price} ₽`}</span>
              <button
                onClick={() => handleQuantityChange(item.id, item.quantity + 1)}
              >
                +
    ...
    <p className="cart-item-price">{item.price} ₽</p>
      ...
      <div className="cart-summary">
        ...
        <button onClick={() => dispatch(setOpen(true))}  className="cart-checkout">Оформить заказ</button>
          </div>
          <Roulette />
    </div>
  );
